...
def to_free_gpu(module):
    t = torch.cuda.get_device_properties(0).total_memory
    a = torch.cuda.memory_allocated(0)
    print(t - a, "<", 1111184384)
    device = 1 if t - a < 1111184384 else 0
    return module.to(f"cuda:{device}")
...
class Transformer(nn.Module):
    def __init__(
        ...
        for d in range(n_depth):
            self._attn_mods.append(to_free_gpu(attn_block(d)))
        self.ws = []
